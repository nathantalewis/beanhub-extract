...
class UssfcuOFXExtractor(ExtractorBase):
    EXTRACTOR_NAME = "ussfcu_ofx"
    DEFAULT_IMPORT_ID = "ussfcu:{{ source_account }}:{{ transaction_id }}"

    def detect(self) -> bool:
        """Detect if this is a USSFCU OFX file."""
        try:
            ...
            # Check for OFX header and USSFCU specific markers
            return (
                "OFXHEADER:" in content and
                "<OFX>" in content and
                "US Senate Federal Credit Union" in content
            )
        except Exception:
            return False
    ...
    def _parse_transactions(self) -> typing.Generator[dict, None, None]:
        ...
        try:
            ...
            print(f"Error parsing USSFCU OFX file: {e}")
            return
    ...
    def __call__(self) -> typing.Generator[Transaction, None, None]:
        ...
        for lineno, txn_data in enumerate(self._parse_transactions(), 1):
            # Validate required transaction fields - fail if missing

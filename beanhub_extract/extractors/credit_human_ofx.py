...
class CreditHumanOFXExtractor(ExtractorBase):
    EXTRACTOR_NAME = "credit_human_ofx"
    DEFAULT_IMPORT_ID = "credit_human:{{ source_account }}:{{ transaction_id }}"

    def detect(self) -> bool:
        """Detect if this is a Credit Human OFX file."""
        try:
            ...
            # Check for OFX header and Credit Human specific markers
            return (
                "OFXHEADER:" in content and
                "<OFX>" in content and
                ("<ORG>Credit Human</ORG>" in content or "<FID>13216</FID>" in content or "Credit Human" in content)
            )
        except Exception:
            return False
    ...
    def _parse_transactions(self) -> typing.Generator[dict, None, None]:
        ...
        try:
            ...
            print(f"Error parsing Credit Human OFX file: {e}")
            return

...
class DocfcuOFXExtractor(ExtractorBase):
    EXTRACTOR_NAME = "docfcu_ofx"
    DEFAULT_IMPORT_ID = "docfcu:{{ source_account }}:{{ transaction_id }}"

    def detect(self) -> bool:
        """Detect if this is a DOCFCU OFX file."""
        try:
            ...
            # Check for OFX header and DOCFCU specific markers
            return (
                "OFXHEADER:" in content and
                "<OFX>" in content and
                "<BANKID>254074439" in content
            )
        except Exception:
            return False
    ...
    def _parse_transactions(self) -> typing.Generator[dict, None, None]:
        ...
        try:
            ...
            print(f"Error parsing DOCFCU OFX file: {e}")
            return
